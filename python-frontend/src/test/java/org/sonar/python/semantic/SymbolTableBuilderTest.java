...
public class SymbolTableBuilderTest {
  ...
  public void comprehension_scope() {
    ...
    assertThat(symbolByName.get("x").usages().stream().map(u -> u.tree().firstToken().line())).hasSize(2).containsExactly(98, 100).doesNotContain(99);
    ...
    assertThat(name.symbol().usages().stream().map(u -> u.tree().firstToken().line())).hasSize(2).doesNotContain(98, 100).containsExactly(99, 99);
  }
